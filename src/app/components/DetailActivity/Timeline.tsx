  ...
  return (
    <ul className="timeline timeline-snap-icon max-md:timeline-compact timeline-vertical">
        ...
        return (
          <li key={index}>
            <div className="timeline-middle">
              <FontAwesomeIcon
                icon={passSchedule ? faCircleCheck : faCircleDot}
                className={passSchedule ? "text-neonBlue" : "text-gray-800"}
              />
            </div>
            <div
              className={`w-fit px-4 inline-block timeline-end ${
                index % 2 === 1 ? "md:text-end md:timeline-start" : ""
              }`}
            >
              <time>{convertDateToThai(data.date)}</time>
              <div className="text-neonBlue text-base md:text-xl">{data.title}</div>
              <div className="px-8 w-fit timeline-end timeline-box mx-0 text-xs md:text-sm inline-block">
                <p>{data.details}</p>
              </div>
            </div>
            <hr
              className={` ${passSchedule ? "bg-neonBlue" : "bg-gray-800"}`}
            />
          </li>
        );
      })}
      <li>
        <div className="timeline-middle">
          <FontAwesomeIcon
            icon={alreadyFinish ? faCircleCheck : faCircleDot}
            className={alreadyFinish ? "text-neonBlue" : "text-gray-800"}
          />
        </div>
        <div
          className={`w-fit px-4 inline-block timeline-end ${
            schedule.length % 2 === 1 ? "md:text-end md:timeline-start" : ""
          }`}
        >
