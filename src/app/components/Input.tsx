  ...
  const renderImageInput = () => {
    ...
    const imageInputElement = (
      <div className={`relative w-full ${!file && "border-dashed aspect-video border-color-primary border-2"} flex justify-center items-center cursor-pointer`}>
        ...
        {file ? (
          <Image  id="selected-image" src={typeof (file) === "string" ? file : URL.createObjectURL(file)} className="w-full object-cover aspect-video object-center" alt="" width={300} height={300} loading="lazy" layout="responsive" />
        ) : (
